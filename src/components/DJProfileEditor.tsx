...
export default function DJProfileEditor({ profile }: Props) {
  ...
  const handleSubmit = async (e?: React.FormEvent) => {
    ...
    try {
      ...
      const updateData = {
        ...
        slug: formData.djName ? slugify(formData.djName) : null,
  ...
  return (
    <form onSubmit={handleSubmit} className="max-w-4xl mx-auto space-y-8">
      <div className="bg-white rounded-lg shadow-sm border p-6">
        <h3 className="text-lg font-semibold text-austin-charcoal mb-4">DJ Profile</h3>

        <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
          <div>
            <label className="block text-sm font-medium text-gray-700 mb-1">DJ Name *</label>
            <Input value={formData.djName} onChange={(e) => setFormData({ ...formData, djName: e.target.value })} required />
          </div>
