...
export default function PhotographerProfileEditor({ profile }: Props) {
  ...
  const uploadFiles = async (files: FileList | null) => {
    ...
    for (const file of Array.from(files)) {
      const bucket = 'photographers'
      const key = `portfolio/${Date.now()}-${file.name}`
  ...
  const handleSubmit = async (e?: React.FormEvent) => {
    ...
    try {
      ...
      const updateData = {
        name: formData.name,
        slug: formData.name ? slugify(formData.name) : null,
  ...
  return (
    <form onSubmit={handleSubmit} className="max-w-4xl mx-auto space-y-8">
      <div className="bg-white rounded-lg shadow-sm border p-6">
        <h3 className="text-lg font-semibold text-austin-charcoal mb-4">Photographer Profile</h3>
        ...
        <div className="bg-white rounded-lg shadow-sm border p-6 mt-4">
          <h3 className="text-lg font-semibold text-austin-charcoal mb-4">Portfolio Photos</h3>
          <div>
            <label className="block text-sm font-medium text-gray-700 mb-2">Upload Portfolio Photos</label>
            ...
            <div className="grid grid-cols-2 md:grid-cols-4 gap-4 mt-4">
              {formData.photos.map((url, idx) => (
                <div key={idx} className="relative">
                  <img src={url} alt={`Portfolio ${idx + 1}`} className="w-full h-28 object-cover rounded-lg" />
